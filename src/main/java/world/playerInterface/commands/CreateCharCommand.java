 ...
 * of deleting the old one. Can be used on a main.java.client that is not logged in, although it just tells them to log in and then quits.
 * Serves as a consuming {@link main.java.network.messaging.MessagePipeline.MessageContext} until the character creation sequence is done.
...
public class CreateCharCommand implements CommandExecutor.Command, MessagePipeline.MessageContext {
    ...
    /**
     * cole constructor
     * @param sourceClient the main.java.client attempting to make a new character
     * @param pipeline the messaging pipeline the main.java.client's commands are parsed through. Used to establish a custom
     *                 MessageContext
     * @see main.java.network.messaging.MessagePipeline.MessageContext
     */
    ...
    /**
     * registers messages from the chosen main.java.client. This class serves as a MessageContext during character creation so
     * messages from the main.java.client come here before parsing. If our main.java.client is talking to us before logging in we do not
